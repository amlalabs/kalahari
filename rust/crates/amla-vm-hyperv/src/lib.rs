...
//! Stub Hyper-V (WHP) backend.
//!
//! Provides the same API shape as the other backends (`amla-kvm`, `amla-hvf`)
//! so `define_backend!(amla_hyperv)` works uniformly on Windows. The WHP
//! implementation has been removed; every constructor returns `Err(VmmError)`.

use std::sync::Arc;

use amla_core::{DeviceWakeIndex, IrqFactory};

/// Hyper-V backend error.
#[derive(Debug, thiserror::Error)]
#[error("hyper-v backend not implemented")]
pub struct VmmError;
...
pub async fn worker_main() -> ! {
    unimplemented!("hyper-v backend not implemented")
}
